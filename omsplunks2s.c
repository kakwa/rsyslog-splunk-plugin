...
static struct cnfparamdescr actpdescr[] = {
    ...
    { "port", eCmdHdlrInt, 0 },
    { "index", eCmdHdlrGetWord, 0 },
    { "host", eCmdHdlrGetWord, 0 },
    { "source", eCmdHdlrGetWord, 0 },
    { "sourcetype", eCmdHdlrGetWord, 0 },
    { "reconnect.interval", eCmdHdlrInt, 0 },
    { "tls", eCmdHdlrBinary, 0 },
    { "tls.verify", eCmdHdlrBinary, 0 },
    { "tls.cacert", eCmdHdlrGetWord, 0 },
    { "tls.cert", eCmdHdlrGetWord, 0 },
...
static int do_connect(instanceData *pData) {
    ...
    if (pData->conn == NULL) {
        LogError(0, RS_RET_SUSPENDED, 
                 "omsplunks2s: failed to connect to %s:%d%s",
    ...
    LogMsg(0, RS_RET_OK, LOG_INFO, 
           "omsplunks2s: connected to %s:%d%s",
           pData->server, pData->port,
           pData->tls_enabled ? " (TLS)" : "");
    return 0;
}
...
CODESTARTdoAction
    ...
    if (err != S2S_OK) {
        LogError(0, RS_RET_SUSPENDED, 
                 "omsplunks2s: send failed: %s", s2s_strerror(err));
        s2s_close(pData->conn);
        pData->conn = NULL;
        pthread_mutex_unlock(&pData->mtx);
        ABORT_FINALIZE(RS_RET_SUSPENDED);
...
BEGINsetActParamDefaults
CODESTARTsetActParamDefaults
ENDsetActParamDefaults
...
CODESTARTnewActInst
    ...
    CHKiRet(OMSRsetEntry(*ppOMSR, 0, NULL, OMSR_NO_RQD_TPL_OPTS));
    ...
    for (i = 0; i < actpblk.nParams; ++i) {
        ...
        if (!strcmp(actpblk.descr[i].name, "server")) {
            ...
            LogError(0, RS_RET_INTERNAL_ERROR,
                     "omsplunks2s: unknown param '%s'", actpblk.descr[i].name);
        }
    }
    ...
    if (pData->server == NULL || pData->server[0] == '\0') {
        LogError(0, RS_RET_CONFIG_ERROR, 
                 "omsplunks2s: 'server' parameter is required");
    ...
    if (do_connect(pData) != 0) {
        LogMsg(0, RS_RET_OK, LOG_WARNING,
               "omsplunks2s: initial connection failed, will retry");
    }
