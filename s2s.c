...
static int conn_write_string(s2s_conn_t *conn, const char *str) {
    ...
    if (conn_write_be32(conn, len) != 0) return -1;
    if (len > 0) {
        if (conn_write_all(conn, str, len) != 0) return -1;
    }
    return 0;
}
...
static int conn_write_kv(s2s_conn_t *conn, const char *key, const char *value) {
    if (conn_write_string(conn, key) != 0) return -1;
    if (conn_write_string(conn, value) != 0) return -1;
    return 0;
}
...
static int do_handshake(s2s_conn_t *conn) {
    ...
    /* Send handshake */
    if (conn_write_all(conn, signature, S2S_SIGNATURE_LEN) != 0) return -1;
    if (conn_write_all(conn, servername, S2S_SERVERNAME_LEN) != 0) return -1;
    if (conn_write_all(conn, mgmtport, S2S_MGMTPORT_LEN) != 0) return -1;
...
s2s_error_t s2s_send(s2s_conn_t *conn, const s2s_event_t *event) {
    ...
    if (event->host && event->host[0]) field_count++;
    if (event->source && event->source[0]) field_count++;
    if (event->sourcetype && event->sourcetype[0]) field_count++;
    if (event->index && event->index[0]) field_count++;
    field_count++; /* _done marker */
    
    /* Write channel ID (0) */
    if (conn_write_be32(conn, 0) != 0) goto send_error;
    
    /* Write field count */
    if (conn_write_be32(conn, field_count) != 0) goto send_error;
    
    /* Write _raw */
    if (conn_write_kv(conn, KEY_RAW, event->raw) != 0) goto send_error;
    
    /* Write _time */
    ts = event->timestamp > 0 ? event->timestamp : time(NULL);
    snprintf(timebuf, sizeof(timebuf), "%ld", (long)ts);
    if (conn_write_kv(conn, KEY_TIME, timebuf) != 0) goto send_error;
    
    /* Write optional fields */
    if (event->host && event->host[0]) {
        if (conn_write_kv(conn, KEY_HOST, event->host) != 0) goto send_error;
    }
    
    if (event->source && event->source[0]) {
        if (conn_write_kv(conn, KEY_SOURCE, event->source) != 0) goto send_error;
    }
    
    if (event->sourcetype && event->sourcetype[0]) {
        if (conn_write_kv(conn, KEY_SOURCETYPE, event->sourcetype) != 0) goto send_error;
    }
    
    if (event->index && event->index[0]) {
        if (conn_write_kv(conn, KEY_INDEX, event->index) != 0) goto send_error;
    }
    
    /* Write _done marker (empty value) */
    if (conn_write_kv(conn, KEY_DONE, "") != 0) goto send_error;
...
const char* s2s_strerror(s2s_error_t err) {
    switch (err) {
        case S2S_OK:               return "Success";
        case S2S_ERR_CONNECT:      return "Connection failed";
        case S2S_ERR_HANDSHAKE:    return "Handshake failed";
        case S2S_ERR_SEND:         return "Send failed";
        case S2S_ERR_MEMORY:       return "Memory allocation failed";
        case S2S_ERR_INVALID:      return "Invalid parameter";
        case S2S_ERR_DISCONNECTED: return "Not connected";
        case S2S_ERR_TLS_INIT:     return "TLS initialization failed";
        case S2S_ERR_TLS_HANDSHAKE: return "TLS handshake failed";
        case S2S_ERR_TLS_CERT:     return "TLS certificate error";
